...
def build_architectural_accessories(bm, props, ctx):
    ...
    # Plot-borne purpose yards (archery field, knights' yard, quarry worksite):
    # enabled by kit toggle OR legacy archetype, laid out in plot space so the
    # hall can be offset/set back without dragging the grounds with it.
    if _kit_on(props, ctx, 'kit_archery', 'ARCHERY_RANGE'):
        build_archery_range(bm, props, ctx, tier)
    if _kit_on(props, ctx, 'kit_tournament', 'KNIGHTS_MANOR'):
        build_tournament_yard(bm, props, ctx, tier)
    if _kit_on(props, ctx, 'kit_quarry', 'QUARRY'):
        from .quarry import build_quarry_yard
        build_quarry_yard(bm, props, ctx, tier)
    # Construction scaffold wraps the translated building in plot space.
    if _prop(props, 'has_construction', False):
        from .scaffold import build_construction_site
        build_construction_site(bm, props, ctx, tier)
...
def _kit_on(props, ctx, kit_name, arch_name):
    """True when a modular kit is enabled via toggle OR legacy archetype."""
    if bool(getattr(props, kit_name, False)):
        return True
    return getattr(ctx, 'effective_archetype', 'NONE') == arch_name


def _build_warehouse_kit(bm, props, ctx, tier, hx, hy, shape, wings, seed):
    yard_x = 0.0
    yard_y = -hy - 1.8
    rot_crane = -1.57
...
def _build_lumbermill_kit(bm, props, ctx, hx, hy, base_w, shape, wings, open_timber, seed):
    ...
    if props.has_front_steps and props.has_foundation and (open_timber or mill_grade == 'GRADE_1'):
        build_front_steps(bm, center_x=mill_sx, y_front=-hy, z_base=ctx.found_h,
                          num_steps=max(2, int(ctx.found_h / 0.18)), normal_axis='-Y')


def build_archetype_accessories(bm, props, ctx, _loft_spec):
    """Modular purpose kits (forge, sails, crane, mill, ...) and loft hatch.

    Each kit fires when its ``kit_*`` toggle is on OR the legacy
    ``building_archetype`` matches, so presets keep working while free-builds
    compose kits freely on any footprint. Kits are independent ``if``s (not
    ``elif``) so several can combine on one building.
    """
    base_w = ctx.base_w
    found_h = ctx.found_h
    hx = ctx.hx

"""
Curved Fairytale Sway Roof Generator with bell-cast flared eaves and sway ridge.
    ...
    for side in [-1, 1]:
        ...
        for k in range(segments_x + 1):
            ...
            if valley_notch is not None:
                _vn = valley_notch
                _notch_side = _vn.get('side', 'BOTH')
                _apply_notch = True
                if _notch_side == 'LEFT' and x_val > _vn['apex_x'] + 0.01:
                    _apply_notch = False
                elif _notch_side == 'RIGHT' and x_val < _vn['apex_x'] - 0.01:
                    _apply_notch = False
                if _apply_notch:
                    _u_val = min(1.0, max(0.0, abs(x_val - _vn['apex_x']) / max(0.001, _vn['half_width'])))
                    _yl = _vn['base_y'] + (_vn['apex_y'] - _vn['base_y']) * (1.0 - _u_val)
                    _overlap = _vn.get('overlap', 0.02)
            ...
            for j in range(segments_y + 1):
                ...
                # Calculate sag using normalized distance along full span
                t_global = max(0.0, min(1.0, (y_val - ry_min) / max(0.01, total_d)))
                sag = math.sin(t_global * math.pi) * sway_amount
        ...
        for k in range(segments_x):
            for j in range(segments_y):
                ...
                for loop in f_top.loops:
                    ...
                    s_dist = math.sqrt((co.x - cx) ** 2 + ((z_base + roof_height) - co.z) ** 2)
                    v_uv = -s_dist * 0.32
                ...
                for ef, kind, top_verts, _k, _j in edge_faces:
                    if kind == 'EAVE':
                        for loop in ef.loops:
                            ...
                            v_coord = (loop.vert.co.y - ry_min) * 0.40
                            loop[uv_layer].uv = Vector((0.0 if is_top else 0.12, v_coord))
                    else: # 'VERGE'
                        for loop in ef.loops:
                            is_top = (loop.vert in top_verts)
                            co = loop.vert.co
                            s_dist = math.sqrt((co.x - cx) ** 2 + ((z_base + roof_height) - co.z) ** 2)
                            loop[uv_layer].uv = Vector((0.0 if is_top else 0.12, s_dist * 0.40))

    # 2. Volumetric Gable End Walls
    half_wt = wall_thickness * 0.5
    rz_ridge = z_base + roof_height

    def get_sway_deck_z(px):
        u = min(1.0, max(0.0, abs(px - cx) / max(0.001, half_w)))
        d = (1.0 - roof_flare) * u + roof_flare * (1.0 - (1.0 - u) ** 2)
        return (rz_ridge - d * (rz_ridge - ez)) - deck_thick - 0.02
